...
const double mu_n = gam_n * h / (2); //neutron magnetic moment
//const double mu_n = gam_n * h / (4 * pi); //neutron magnetic moment
const double v = 4.91; //m/s
const double d = 1e-3; //m
//const double w = 2 * pi * v / d; //rad/s
const double w = 2660.0; //rad/s
double dt;
double TF;
double t;
int m;
...
const int N = 4; //number of states/size of a
...
ArrayXXd B(double t, ArrayXd Bh, ArrayXXd Bo)
{
        ...
        if (m % 2 == 0) {
            ...
            for (l = 0; l < N; l = l + 2) {
                if (l == m) {
                    ...
                    HBpm = -(B(0, 0) + B(1, 0) * (2.0 / 3.0) * z(m) + B(2, 0) * (4.0 / 15.0) * pow(z(m), 2));
                }
                else {
            ...
            for (l = 1; l < N; l = l + 2) {
                if (l == m) {
                    ...
                    HBmp = -(B(0, 0) + B(1, 0) * (2.0 / 3.0) * z(m) + B(2, 0) * (4.0 / 15.0) * pow(z(m), 2));
                }
                else {
